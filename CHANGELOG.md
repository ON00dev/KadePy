...
### Changed
- **Architecture Overhaul**: Migrated KadePy to use the official Hyperswarm stack (via `hyperswarm`, `hyperdht`, `dht-rpc`, `libudx`) through a native Node.js daemon binding.
  - Ensures 100% protocol compliance and wire compatibility with the Hyperswarm ecosystem.
  - Replaces previous custom C implementation of Noise/UDX/DHT with official libraries.
- **Dependency Management**: Vendored official repositories in `vendor/` pinned to specific versions (hyperswarm v4.16.0, hyperdht v6.28.0) to guarantee reproducible builds.
- **Python API**: Updated `Swarm` class to interface with the new `NativeHyperswarm` backend.

## [0.2.4] - 2026-01-25

### Changed
- **API Update**: `Swarm.join()` now accepts `announce` and `lookup` boolean parameters (default: `True`) for fine-grained DHT control.
- **Validation**: Enforced strict 32-byte length check for topics in `Swarm.join()` and the Node.js bridge to ensure Hyperswarm compatibility.
